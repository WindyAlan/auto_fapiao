...
2. **自动校验发票信息** — 把Excel里填的发票号、日期、金额等信息和PDF里OCR识别出来的结果对比，找出不一致的地方，生成报告告诉你哪些需要手动修改。

> **非技术用户？** 请直接看 [用户手册](用户手册.md)，里面有详细的操作步骤。
...
- 生成 `rename_report.txt` 报告，列出成功/跳过/失败的文件及原因
...
- 用OCR识别每张PDF发票的实际内容
- 逐项对比：发票号、日期、税金、含税金额
- 生成 `verify_report.txt` 报告，告诉你哪些对、哪些有差异（原Excel不做任何修改）
...
**Q: 如果OCR识别不准怎么办？**
A: 报告里会标记"需手动核查"，你对照 PDF 发票原件，手动修改 Excel 即可。
